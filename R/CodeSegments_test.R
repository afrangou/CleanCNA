...
CodeSegments_test <- function(segfile_dir,segfile_name) {

  print("codesegments_test")
  ...
  # homdels
  subs$coded_total_cn[which(subs$total_cn==0)] = "homdel_test"
  # LOH
  subs$coded_total_cn[which(subs$nMajor>0 & subs$nMinor==0)] = "loh_test"
  # other loss (non homdel, non LOG)
  subs$coded_total_cn[which(subs$total_cn < subs$dip.tetra
                            & subs$total_cn != 0
                            & subs$nMinor != 0)] = "otherloss_test"
  # no change from ploidy class
  subs$coded_total_cn[which((subs$nMajor==subs$nMinor | subs$nMajor==3 & subs$nMinor==1)
                            & (subs$total_cn == subs$dip.tetra))] = "nochange_test"
  # gain
  subs$coded_total_cn[which(subs$total_cn>subs$dip.tetra)] = "gain_test"
  # big gain
  subs$coded_total_cn[which(subs$total_cn>(5*subs$dip.tetra))] = "biggain_test"
